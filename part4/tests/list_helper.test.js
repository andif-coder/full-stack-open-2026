...
describe('total likes', () => {
	...
	test('when list has only one blog, equals the likes of that', () => {
		const ret = listHelper.totalLikes(listWithOneBlog)
	...
	test('of a bigger list is calculated right', () => {
		assert.strictEqual(listHelper.totalLikes(listWithMultBlogs), 36)
	})
})
describe('favorite', () => {
	test('mult blogs', () => {
		const ret = listHelper.favoriteBlog(listWithMultBlogs)
		assert.deepStrictEqual(ret, listWithMultBlogs[2])
	})
	test('empty blogs', () => {
		const ret = listHelper.favoriteBlog([])
